    ...
    PosatoAppScaffold(
        ...
        Box(Modifier.fillMaxSize(), contentAlignment = Alignment.TopCenter) {
            key(state.surface, state.editor.sessionKey) {
                Column(
                    modifier = Modifier.widthIn(max = PosatoSize.Content).fillMaxWidth().verticalScroll(rememberScrollState())
...
private fun PrototypeSurface(
    ...
    when (state.surface) {
        ...
        PrototypeSurface.Targets, PrototypeSurface.Items -> PrototypeItems(state, layout, onAction)
