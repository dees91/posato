...
# Posato Native Prototype Design

## Design Intent

This describes the implemented native prototype as of **2026-09-06**,
documented at the maintainer's request. It is a reproduction reference for the
mock, not the accepted production contract. [The root DESIGN.md](../../DESIGN.md)
remains authoritative for product work; production-boundary summaries below are
context, not a second authority. Source paths in the front matter are repository-relative.
...
[native prototype](README.md). Its shared
[component library](compose/README.md) is the
...
[complete component map](compose/README.md#complete-prototype-component-map)
...
- `Observed` / `user-confirmed` (2026-09-06): the maintainer requested an
  accurate description of the current prototype, originally in the root
  `DESIGN.md`, after
  refining duration entry, batch website entry, list/menu hierarchy, category
  controls, and native main navigation.
- `Observed`: the [Compose design-system sources](compose/src/commonMain/kotlin/app/posato/prototype/designsystem/)
  declare the palettes, type roles, spacing, shapes, semantics, and components
  described here. The [screen sources](app/src/commonMain/kotlin/app/posato/prototype/ui/)
  determine which components the current application actually uses.
- `Observed`: the [desktop host](app/src/jvmMain/kotlin/app/posato/prototype/Main.kt)
  supplies the native window and minimum width; the [SwiftUI host](iosApp/PosatoPrototype/PosatoPrototypeApp.swift)
