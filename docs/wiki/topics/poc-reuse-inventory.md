...
| Encrypted envelope and deterministic vectors | Reference and selectively port tests | Common E2EE boundary and cross-runtime interoperability technique for CloudKit and portable folders | Select reviewed primitives, providers, formats, versioning, and mode-specific key handling |
...
| Enrollment, recovery, membership, revocation, and deletion flows | Retain as portable-mode design evidence | Explicit lifecycle states, QR replay protection, per-device wrapping, key epochs, and failure cases | Do not import them into Apple onboarding; redesign them for the later portable threat model and UX |
...
| Native CloudKit and secure-storage adapters | Reference as platform leaves | Practical CloudKit transport, synchronizable-Keychain delivery, and error mapping | Implement the one-action Apple contract, deterministic missing-key waiting state, account isolation, and current production boundaries |
