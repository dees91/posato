...
| `TARGETS-006` | Explain exact-host coverage when a website is entered and offer an explicit way to include its `www` variant, keeping the ADR 0005 exact-domain contract and recording separately whether broader subdomain coverage is wanted. | Target management | delivery | R1.1/W1 | None | PR-WWW-COVERAGE |
