...
2. Replace the zone-deletion operation on the mailbox port with record
   deletion plus absence verification; implement it in the companion
   (protocol code, request handler, store, backend) and in the iOS provider
   and live backend, in bounded idempotent batches.
3. Add `ANCHOR_MISSING` to the established check, map it in `AppleSync`,
   the coordinator, and removal (skip record deletion, delete own key, clear,
   tombstone); keep `DIFFERENT_ANCHOR` unchanged.
4. Map deletion entries to progress-only pages in both adapters and cover
   the page in the exchange tests.
