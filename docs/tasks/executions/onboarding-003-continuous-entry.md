...
- After the maintainer rejected the clipped field in the iPad mini landscape screenshot, expanded pages use 24 dp vertical margins and gap before the actions while the keyboard is up, and `WebsiteStep` brings the field and its total into view with a `BringIntoViewRequester` whenever the keyboard height changes. Compose alone scrolled only the caret line into view.
...
- Keyboard-clearance review (independent agent): no Critical or Required findings. Recommended: when the field-and-total group is taller than the viewport (iPhone landscape), bring-into-view could align to the group's lower edge after an unusual prior scroll. The run shows top alignment; the edge case is accepted.
...
- After the keyboard-clearance fix: iPad mini `20260923-090737-dea3` shows the whole field, the last submission's feedback, `2 websites saved`, and **Continue** above the keyboard in landscape, with portrait unchanged apart from the 24 dp top margin while typing. iPhone 17 `20260923-090852-e3b2` shows compact portrait unchanged; in landscape the whole field and **Continue** are visible, and a third addition brings the total to 3 and Summary to `3 websites saved`. The Mac has no software keyboard and is unaffected.
...
- On iPhone landscape with the keyboard up, the content viewport is about 80 pt: the whole field and **Continue** are visible, while the last submission's feedback is partly clipped and the total sits below it, reachable by scrolling. iPad mini landscape now shows the field, the feedback, the total, and **Continue** together.
