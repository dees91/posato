...
- README leads with the identity, a value sentence, and an animated demo, followed by a short source-build route, illustrated user steps, and documentation links. Following maintainer feedback, full build instructions live in the existing development guide, and the availability table, full limits, supported platforms, and platforms planned for later releases live in the user-facing `docs/product/limits-and-platforms.md`; the original limits are preserved verbatim there. Root privacy, support, documentation, and license sections remain unchanged.
...
- Local README/media links resolve. A comparison confirms the full limits survive verbatim in `docs/product/limits-and-platforms.md` and root privacy-related sections remain unchanged. GitHub's Markdown API rendered the README, and browser inspection loaded every image; the correction also passes local link/anchor checks and GitHub Markdown rendering.
