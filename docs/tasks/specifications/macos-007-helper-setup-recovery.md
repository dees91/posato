...
- `AC-03` — Onboarding and This Mac show progress immediately and present only an actionable outcome. Not now works during a delayed Mac call; Summary/Session receive its eventual state without auto-advancing or starting enforcement. UI copy distinguishes uncertainty from confirmed failure, preserves the collapsed Session layout, and does not promise a duration or that restarting fixes service registration.
