...
- **Tier reason:** Brand assets, screenshots, and one read-only shared screen change no security, privacy, or synchronization boundary; an independent check still covers design fidelity and the new navigation entry.
...
Posato has an application icon on iOS and macOS derived from its open-interval mark, App Store screenshots and listing copy the maintainer has accepted, and a licenses screen on both platforms that shows the license, notice, and third-party notices shipped with the app.
...
- The licenses screen reads the repository's `LICENSE`, `NOTICE`, and `THIRD_PARTY_NOTICES.md` as shared resources produced from those files at build time, never from a second hand-maintained copy. Amend DESIGN.md with its entry point and layout within the existing components and voice.
...
- `AC-03` — On both platforms the licenses screen is reachable from the entry point DESIGN.md names and shows the three texts in full, scrollable, with accessible labels.
...
- **Open:** the licenses screen entry point, proposed in the DESIGN.md amendment for maintainer acceptance.
