...
| `TB-06` Applications and macOS sync companion → CloudKit and synchronizable Keychain | Encrypted bundles, bounded metadata, workspace key | Treat transport input as hostile; match the expected local-only account token around every private-CloudKit operation and bootstrap Keychain operation; isolate accounts and exact resources; represent unavailable/delayed services truthfully | `SYNC-003`, `SYNC-005`–`SYNC-010` |
