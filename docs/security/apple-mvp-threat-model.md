...
| `TB-06` Applications and macOS sync companion → CloudKit and synchronizable Keychain | Encrypted bundles, bounded metadata, workspace key | Treat transport input as hostile; match the expected local-only account token before and after every bootstrap operation; isolate accounts and exact resources; represent unavailable/delayed services truthfully | `SYNC-003`, `SYNC-005`–`SYNC-010` |
...
| `T-04` | `S/T/D` | CloudKit workspace state races delayed Keychain delivery, account change, reset, entitlement mismatch, or concurrent first run | Deterministic one-workspace bootstrap, exact Keychain item, durable local-only account binding checked before and after provider access, waiting state, fail-closed account change, restart-safe retry, and never a replacement key or parallel workspace | `SYNC-003`–`SYNC-006`, `SYNC-009` | Losing all workspace-key access is irreversible in the MVP |
