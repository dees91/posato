...
| `A-04` | Immutable operations, projected policy, pending publication, cursor, policy base and intents, session intents and origin, bootstrap state, and device-local removed-workspace identifiers | Preserve the local-first replica and retry work | App-private database; complete encrypted bundles cross the cloud boundary; removed-workspace identifiers stay local, unsynchronized, and undiagnosed | Security state: bounded, authenticated, atomic, idempotent, migration-safe, and never silently reset after corruption | `MODEL-001`, `SYNC-002`, `SYNC-004`, `SYNC-010`, `SYNC-011`, `SYNC-012`, `SYNC-014` |
...
| `A-06` | Encrypted mailbox bundles and bounded routing fields | Exchange immutable operations through CloudKit Private Database | Private CloudKit records and local transport state | Protected payload plus provider-visible metadata: authenticate context, bound fields and size, minimize metadata, reject unknown versions | `SYNC-001`, `SYNC-007`, `SYNC-008`, `SYNC-015` |
...
| `TB-04` Desktop JVM application → signed native helper | Enforcement commands and structured results over local IPC | Fixed helper, peer authentication and authorization, bounded versioned allowlist, freshness/replay defense, timeouts, redacted failures; an opted-in standing Apply grant bound to the peer's user, account, Mac, and console session, with no bearer material (ADR 0004 `MACOS-014` amendment) | `MACOS-001`, `MACOS-003`, `MACOS-014` |
...
| `TB-06` Applications and macOS sync companion → CloudKit and synchronizable Keychain | Encrypted bundles, bounded metadata, workspace key | Treat transport input as hostile; match the expected local-only account token around every private-CloudKit and synchronizable workspace-key operation; isolate accounts and exact resources; represent unavailable/delayed services truthfully | `SYNC-003`, `SYNC-005`–`SYNC-010` |
...
| `TB-08` Source/dependency/signing chain → installed process | Code, build tools, dependencies, entitlements, signatures, updates | Review and pin inputs, minimize entitlements, verify identities and artifacts, keep credentials outside source and routine CI; Mac updates accept only a Sparkle feed and archive whose EdDSA signatures verify against the key embedded in the running build, from a notarized Developer ID release whose feed the release task validated before publication | `QUALITY-001`, `CI-001`, target owners, `RELEASE-002`, `MACOS-011`, `RELEASE-003` |
| `TB-09` Desktop JVM application → signed synchronization companion | Workspace-key records, opaque mailbox bytes, bounded routing metadata, local-only account binding, and semantic outcomes over private IPC | Fixed embedded peers, mutual signed-relationship checks, bounded versioned allowlist, request identity, timeouts, redacted outcomes, and no enforcement or general command surface | `SYNC-003`, `SYNC-006`, `SYNC-008` |
...
| `T-03` | `T/R` | Duplicate, replayed, reordered, stale, rolled-back, corrupt, or partially stored data regresses policy or loses pending work, reuses a local author sequence, overflows HLC, or lets wall-clock rollback revive an observed-expired session | Fresh process-memory authoring incarnations, retained replay/sequence state, exact ambiguous-commit reconciliation, serialized local/remote checkpoint advancement and same-open footprint checks, fail-closed HLC exhaustion, deterministic full reduction, conflicting-start quarantine, terminal local expiry keyed to the session identifier, one atomic apply, base-kept converged apply with durable intents, durable session intents coupled to the local session commit, session adoption by identity with receiver-side frozen summary, and no silent empty initialization | `MODEL-001`, `SESSION-001`, `SESSION-002`, `SYNC-002`, `SYNC-004`, `SYNC-010`, `SYNC-011`, `SYNC-012` | A restored or fresh replica cannot infer data or a local expiry observation absent from every retained source; a terminal HLC blocks local authoring, and Apple MVP availability still depends on surviving data |
| `T-04` | `S/T/D` | CloudKit zone or workspace state races delayed Keychain delivery, account change, reset, entitlement mismatch, or concurrent first run | Deterministic one-workspace bootstrap, binding-checked exact-zone establishment before anchor absence, action-required zone loss after establishment, exact Keychain item, durable local-only account binding checked around bootstrap, every workspace-key operation, destructive removal, and ongoing mailbox access, binding-scoped sync-engine invalidation, waiting state, fail-closed account change, restart-safe retry, a device-local tombstone that refuses re-adoption of a workspace this device removed, and never a replacement key, parallel workspace, cross-account read or deletion, cross-account bundle acceptance, or false publication acknowledgement | `SYNC-003`–`SYNC-010`, `SYNC-014` | Losing all workspace-key access is irreversible in the MVP |
...
| `T-13` | `S/T/I/E` | A compromised dependency, build input, signing path, update, or over-entitled binary defeats application checks | Minimal reviewed dependencies; pinned reproducible inputs; credential-free CI; least entitlements; native-process/parent identity checks; signing, update, license, and security review before release; Sparkle pinned by SHA-256, required signed feed and archive, update key only in the maintainer's Keychain with an encrypted backup outside the repository, and Sparkle's installer admitted only after the maintenance gate closes | `QUALITY-001`, `CI-001`, every dependency/target owner, `RELEASE-002`, `MACOS-011` | Compromised installed code has the plaintext and privileges of its process; Posato does not claim protection from itself |
| `T-14` | `T/I/D/E` | Reset, deletion, uninstall, account change, or recovery removes unrelated resources, leaves protected data behind, or is described as remote wipe | Exact namespaces and selectors; explicit destructive intent; idempotent deletion; independent absence and outside-scope preservation checks; safe account isolation and uninstall recovery; a device-local tombstone so this device does not re-adopt the workspace it just removed while CloudKit still surfaces it; `SYNC-015` record-based removal (bundles first, anchor last, looped absence verification, the zone is never deleted), fresh-attempt sweep while no anchor exists, and foreign-context skip | `MODEL-001`, `SYNC-003`, `SYNC-005`–`SYNC-010`, `SYNC-014`, `SYNC-015`, `MACOS-001`, `IOS-001`, `IOS-002`, `RELEASE-001` | Provider or backup retention may outlive immediate deletion; data already copied to another device cannot be remotely erased; a fresh install without a tombstone can still observe recently deleted records during provider propagation (narrowed to provider anomalies); no zone purge exists, so the purge-window residual is closed, and the late-publish window is closed by the foreign-context skip |
...
`SYNC-004` through `SYNC-010` must still implement and prove it. Diagnostic producers remain unimplemented and must satisfy the
...
`observed` (2026-09-25, `MACOS-011`): Mac update checks and downloads reveal
to GitHub and its delivery providers the IP address, request time, requested
resource (an artifact address identifies the version), the fixed User-Agent
`PosatoUpdater`, `Accept-Language: en`, and connection metadata. They carry no
cookie, installation identifier, system profile, or product data, and the
updater refuses GitHub's cookies. They occur only after consent or a manual
check. This provider-visible metadata falls under `R-03`.
...
`user-confirmed` (2026-09-11): `SYNC-014` adds a device-local tombstone of
removed workspace identifiers only. It adds no transport, key, or diagnostic
surface and strengthens `T-04` and `T-14` on the device that performed the
removal. A fresh install without that row can still join a ghost during the
provider purge window, and a fresh workspace established minutes after
removal can be lost when the provider purges the same-name zone; the
`SYNC-014` physical gate observed that second case once, and the fix
