...
| `A-05` | Workspace key, signing identity, Keychain selectors, and ephemeral IPC authentication material | Decrypt/authenticate the workspace and protected local channels | Exact secure-storage item selected by the owning task or process memory; only the workspace key may use synchronizable Keychain | Secret: least-accessible compatible storage, no plaintext fallback, arguments, environment, logs, or public artifacts | `SYNC-001`, `SYNC-003`, `SYNC-005`, `SYNC-006`, `MACOS-003` |
...
| `A-08` | Diagnostic, crash, support, and test evidence | Explain failures without reconstructing behavior | No production collection or export until `DIAGNOSTICS-001`; synthetic tracked evidence only | Allowlisted support data: no domains, applications, accounts, devices, keys, tokens, URLs, content, browsing or usage events | `DIAGNOSTICS-001`, every producing task, `RELEASE-001` |
...
| `T-03` | `T/R` | Duplicate, replayed, reordered, stale, rolled-back, corrupt, or partially stored data regresses policy or loses pending work | Stable identities and sequencing, replay/high-water checks, deterministic reduction, one atomic apply, durable retry, and no silent empty initialization | `MODEL-001`, `SYNC-002`, `SYNC-004`, `SYNC-010` | A fresh replica cannot infer data it has never observed; Apple MVP availability still depends on surviving data |
...
format, automatic signed-author registration, data retention/deletion, and
diagnostic fields remain blocked on their named tasks. `RELEASE-001` must
recheck both the residuals and those feature gates before any public-release
claim.
