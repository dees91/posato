...
| `T-13` | `S/T/I/E` | A compromised dependency, build input, signing path, update, or over-entitled binary defeats application checks | Minimal reviewed dependencies; pinned reproducible inputs; credential-free CI; least entitlements; native-process/parent identity checks; signing, update, license, and security review before release; Sparkle pinned by SHA-256, required signed feed and archive, update key only in the maintainer's Keychain with an encrypted offline backup, and Sparkle's installer admitted only after the maintenance gate closes | `QUALITY-001`, `CI-001`, every dependency/target owner, `RELEASE-002`, `MACOS-011` | Compromised installed code has the plaintext and privileges of its process; Posato does not claim protection from itself |
