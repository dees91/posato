...
| `A-04` | Immutable operations, projected policy, pending publication, cursor, policy base and intents, and bootstrap state | Preserve the local-first replica and retry work | App-private database; complete encrypted bundles cross the cloud boundary | Security state: bounded, authenticated, atomic, idempotent, migration-safe, and never silently reset after corruption | `MODEL-001`, `SYNC-002`, `SYNC-004`, `SYNC-010`, `SYNC-011` |
