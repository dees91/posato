...
- `mapping-empty` shows `No applications chosen on this Mac.` when none is chosen.
...
- **Empty state.** Read the section. Run `$PC find -t desktop --text "No applications chosen on this Mac."`; one text element is returned.
...
- **List and remove.** After a manual pick, run `$PC snapshot -t desktop --format text --human`; a row with the application name and a `Remove` button appears. Run `$PC tap -t desktop --text Remove --role button --near-text "<application name>"`; `wait --for exists --text "No applications chosen on this Mac."` returns `ok`.
