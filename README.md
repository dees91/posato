...
those websites and apps until the session ends or you deliberately end it early.
With **Sync with iCloud**, a session started on one device reaches your other
device.
...
- **Run a manual session** with a chosen duration. It ends on its own at the
  selected time or when you deliberately end it early.
...
- On macOS, website blocking covers **Safari** and **Google Chrome Stable** for
  ...
  proxy are not covered. Pages already loaded, cached, or downloaded are not
  erased.
...
- A Mac with Apple silicon, Xcode, and an iOS Simulator runtime.
- Java 17 or later to launch Gradle; the build downloads the JDK 21 it uses.
...
- Report bugs and ask questions in [GitHub Issues](../../issues). Please do not
  include website names, app names, device details, screenshots, or logs.
