...
private const val HTTP_REDIRECT_LAST = 399
...
internal fun classifyPage(
    ...
    status in HTTP_OK_FIRST..HTTP_REDIRECT_LAST -> PageOutcome.LOADED
...
class BlockingObserver(
    ...
    fun website(url: String): WebsiteObservation {
        ...
        val command = buildList {
            addAll(listOf("/usr/bin/curl", "-s", "-m", CURL_SECONDS.toString(), "-w", "\n%{http_code}"))
            proxy?.let { addAll(listOf("--proxy", "http://${it.host}:${it.port}")) }
            add(url)
        }
        val (body, status) = splitCurlOutput(context.subprocess.run(command, timeout = Duration.ofSeconds(CURL_SECONDS + GRACE_SECONDS)).stdout)
        return WebsiteObservation(url, proxy != null, status, classifyPage(status, body).name.lowercase())
    }

    /** Opens the application and reports whether it still runs after [seconds]; a paused one is ended at launch. */
    fun application(
        name: String,
        seconds: Int
    ): ApplicationObservation {
        context.subprocess.run(listOf("/usr/bin/open", "-a", name))
            .requireSuccess(ErrorCode.COMMAND_FAILED, "Opening $name", "Name an installed application, such as Safari.")
        Thread.sleep(Duration.ofSeconds(seconds.toLong()).toMillis())
        val running = context.subprocess.run(listOf("/usr/bin/pgrep", "-x", name)).succeeded
        if (running) context.subprocess.run(listOf("/usr/bin/pkill", "-x", name))
        return ApplicationObservation(name, seconds, running)
    }

    private companion object {
        const val CURL_SECONDS = 15L
